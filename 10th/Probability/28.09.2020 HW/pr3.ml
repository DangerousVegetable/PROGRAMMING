...
let rec wlength in_chan = 
	let st = List.of_seq (Hashtbl.to_seq (stats in_chan 1)) in
	List.fold_left (fun s ((w,_),n) -> s+(List.length w)*n) 0 st;; 
...
let dirstats dir =
	...
	let n = float_of_int (List.fold_left (fun s name -> let i = open_in_bin name in s+(total_words i)) 0 file_names) in
	List.fold_left (fun s name -> let i = open_in_bin name in s+.(float_of_int(wlength i))/.n) 0. file_names;;
