...
let dirstats dir = 
	...
	let file_stats = List.map (fun name -> let in_chan = open_in_bin name in let s = stats in_chan 1 in close_in in_chan; (name,s)) file_names in
	file_stats;;
...
(*match l with
|[] -> failwith "damn..."
|tbl::_ -> List.iter (fun ((x,_),n) -> Printf.printf "Word: [%s] - %n \n" (List.fold_left (fun s x -> s^(string_of_int x)^";") "" x) n) (Seq.fold_left (fun l a -> a::l) [] (Hashtbl.to_seq tbl));; *)



	

(*print_int (List.fold_left (fun s (_,n) -> s+n) 0 (stats 1));;        *)
