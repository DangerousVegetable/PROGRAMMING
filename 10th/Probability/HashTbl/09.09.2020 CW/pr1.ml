...
let fsort = fun ((w1,q1),n1) ((w2,q2),n2) ->
			if q1 = q2 then
				if (is_proper w1) = (is_proper w2) then 		 
					if n1 = n2 then 
						if w1 <! w2 then -1 else 1 
					else if n1>n2 then -1 else 1 
				else if is_proper w1 then -1 else 1  
			else if q1 then 1 else -1;;
			
List.iter (fun (x,k) -> List.iter (fun x -> List.iter (fun x -> output_byte out_chan x) (to_utf8 x)) x; output_string out_chan (" - "^(string_of_int k)^"\n")) (List.map (fun ((w,q),n) -> if q then (w,n) else (w,n)) (cut am (List.sort fsort (List.filter (fun ((w,q),n) -> (not q)&&(is_proper w)) (stats len)))));;		                      
