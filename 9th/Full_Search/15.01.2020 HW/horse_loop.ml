...
let solve n = 	
	...
	let rec main step col row visited = 
		if step >= n*n-1 then if (col = 1 && row = 2)||(col = 2 && row = 1) then raise (Found visited) else () else List.iter (fun (_,x,y) -> main (step+1) x y ((x,y)::visited)) (stp col row visited 0 []) in 
